                ...
                article.innerHTML =
                    '<div class="cm-message-card__header">' +
                        '<div class="cm-avatar cm-avatar--card cm-avatar--' + (rec.avatar_color || 'violet') + '">' +
                            (rec.profile_image ? '<img src="' + (rec.profile_image.indexOf('http') === 0 ? escapeHtml(rec.profile_image) : (window.APP_BASE_URL || '') + '/' + escapeHtml(rec.profile_image)) + '" alt="" class="cm-avatar__img" width="48" height="48">' : (rec.initials || '?')) +
                        '</div>' +
            ...
            document.querySelectorAll('[data-cm-action="view"]').forEach(function (btn) {
                btn.onclick = function () {
                    ...
                    if (value('data-profile-image')) {
                        ...
                        if (avatarImg) {
                            avatarImg.src = (value('data-profile-image').indexOf('http') === 0) ? value('data-profile-image') : (window.APP_BASE_URL || '') + '/' + value('data-profile-image');
