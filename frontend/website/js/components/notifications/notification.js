/**
 * SOUND Group — Website Notification Component
...
(function () {
    ...
    function getContainer() {
        var existing = document.getElementById('wg-notification');
        if (existing) {
            return existing;
        }

        var el = document.createElement('div');
        el.id = 'wg-notification';
        el.className = 'wg-notification';
        el.setAttribute('role', 'alert');
        el.setAttribute('aria-live', 'assertive');
        el.innerHTML = '<span class="wg-notification__icon"></span><span class="wg-notification__message"></span>';
    ...
    function show(type, message, duration) {
        var el = getContainer();
        var icon = el.querySelector('.wg-notification__icon');
        var text = el.querySelector('.wg-notification__message');

        clearTimeout(el._hideTimer);
        clearTimeout(el._exitTimer);

        el.className = 'wg-notification wg-notification--' + type;
